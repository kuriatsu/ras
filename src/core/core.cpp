...
void RasCore::subObjCallback(const derived_object_msgs::ObjectArray &in_obj_array)
{
	...
	for (const auto &e : in_obj_array.objects)
	{
		...
		if (m_min_vision < ras_obj.distance && ras_obj.distance < m_max_vision && ras_obj.object.id != m_ego_id)
		{
			...
			selected_obj.is_touched = (in_obj_pose.position.x < 0.5) ? false; // reset operation if obstacle over takes ego_vehicle
...
//             }
//
//             CalcVector obj_closewp_vec(in_obj.object.pose.position, m_wps_vec[close_wp].position);
...
// bool RasCore::isSameDirection(const CalcVector &vec_1, const CalcVector &vec_2, const float &thres)
// {
...
//
//
// bool RasCore::isPerpendicular(const CalcVector &vec_1, const CalcVector &vec_2, const float &thres)
// {
//     float inner_prod = vec_1.x * vec_2.x + vec_1.y * vec_2.y;
